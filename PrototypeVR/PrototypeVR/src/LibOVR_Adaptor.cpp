...
void LibOVR_Adaptor::Initialize(void)
{
	ovr_Initialize();

	//Initialize the HMD device connected to the computer.
	hmd = ovrHmd_Create(0);

	if (hmd)
	{
		///Get the Data from the HMD.
		ovrSizei resolution = hmd->Resolution;

		///Setting up the Texture for rendering the scene!
	...
	{
		///Initialize Debug HMD for Rendering Testing.
	...
	:IPD_Unit_Conversion( 512.5 )
{
	...
	if (pHMD) // Check if device was found.
	{
